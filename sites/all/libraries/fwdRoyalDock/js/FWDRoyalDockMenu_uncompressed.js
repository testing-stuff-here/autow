...
(function (window){
	...
	var FWDDisplayObject = function(type, position, overflow, display){
		...
		this.removeChild = function(e, parent){
      var s = this.screen.parentNode;
      
      // For some of the elements it won't work with parentNode
      if(parent == 'screen') {
        var s = this.screen;
      }
			...
			if(this.contains(e)){
				this.children_ar.splice(FWDUtils.indexOfArray(this.children_ar, e), 1);
				if(this.childOf(e.screen, s)) {
  		    s.removeChild(e.screen);
  		  }
			}else{
		...
		this.childOf = function(child, parent){
      ...
      return !!child; 
	   ...
	   this.removeListener = function(type, listener){
		...
		this.destroy = function(){
			...
			try{this.screen.removeChild(this.screen);}catch(e){};
...
(function (window){
	 ...
	 * @ display values: block, inline-block, this applies only if the position is relative.
	 */
	var FWDDisplayObjectScale = function(type, position, overflow, display){
		...
		this.setX = function(val){
			...
			if(self.hasTransform2d_bl){
				self.screen.style[self.transform] = "translate(" + self.x + "px," + self.y + "px) scale(" + self.scale + " , " + self.scale + ") rotate(.01deg)";
			}else{
				self.screen.style.left = self.x + "px";
			}
		};
	...
	window.FWDDisplayObjectScale = FWDDisplayObjectScale;
}(window));(function (){
	...
	var FWDLightBox = function(props){
		...
		this.show = function(id){
			...
			if(navigator.userAgent.toLowerCase().indexOf("msie 7") != -1){
				parent.document.getElementsByTagName("body")[0].appendChild(this.screen);
			}else{
				parent.document.documentElement.appendChild(this.screen);
		...
		this.showCurrentItem = function(){	 
			...
			if(this.data_ar[this.id].dataType.toLowerCase() == "iframe"){
				...
				if(!args.v){
					this.addChild(this.info_do, true);
				...
				if(this.firstTimeShowed_bl){
					...
					if(this.showZoomButton_bl && (this.type_str == FWDLightBox.YOUTUBE || this.type_str == FWDLightBox.VIMEO || self.type_str == FWDLightBox.IFRAME)){
						...
						if(index != -1){
							...
							this.removeChild(this.zoomButton_do, 'screen');
						}
					}
		...
		this.createIframeHolder = function(){
			...
			this.itemsHolder_do.addChild(self.currentItem_do, true);
		};
		...
		this.loadImage = function(){
			...
			this.addChild(this.preloader_do, true);
		};
...
(function(window)
{
	var FWDRDMData = function(props)
	{
		var self = this;
		var prototype = FWDRDMData.prototype;
		...
		this.parseProperties = function()
		{
			...
			// check for dockMenuDataListDivId property.
			if (!self.propsObj.dockMenuDataListDivId)
			{
				errorMessage = "DockMenu data list id is not defined in FWDRDMDockMenu constructor function!";
				self.dispatchEvent(FWDRDMData.LOAD_ERROR, {text : errorMessage});
				
				return;
			};

			// set the root element of the dockMenu list.
			self.rootElement = FWDUtils.getChildById(self.propsObj.dockMenuDataListDivId);
			
			if (!self.rootElement)
			{
				errorMessage = "Make sure that the div with the id <font color='#FFFFFF'>" + self.propsObj.dockMenuDataListDivId + "</font> exists, this represents the dockMenu data list.";
				self.dispatchEvent(FWDRDMData.LOAD_ERROR, {text : errorMessage});
			...
			self.thumbMinScale = self.propsObj.thumbnailsMinimumScale || .5;
			self.thumbSpace = self.propsObj.thumbnailsSpace || 10;
			self.sideSpace = self.propsObj.thumbnailsSideSpace || 10;
			self.inflDist = self.propsObj.thumbnailsInfluenceDistance || 400;
			...
			self.maxNumberOfThumbsOnMobile = self.propsObj.maxNumberOfThumbnailsOnMobile || 20;
			...
			self.menuStyle = self.propsObj.menuStyle == "yes" ? true : false;
			...
			self.autoplaySpeed = self.propsObj.autoplaySpeed || 20;
			...
			for (var i=0; i<totalDataLists; i++)
			{
				...
				for (var j=0; j<totalChildren; j++)
				{
					...
					if (self.showTooltip)
					{
						// check for data-tooltip-text attribute.
						var hasError = true;
						
						for (var k=0; k<totalInnerChildren; k++)
						{
							attributeMissing = "data-tooltip-text";
							
							if (FWDUtils.hasAttribute(childKidsAr[k], "data-tooltip-text"))
							{
								hasError = false;
			...
			{
				self.dispatchEvent(FWDRDMData.LOAD_ERROR, {text:"DockMenu graphics skin path is not defined in FWDRDMDockMenu constructor function!"});
				return;
			}
			
			//set dockMenu graphics.
			...
			try
			{
				self.rootElement.removeChild(self.rootElement);
			}
			catch(e){};
		...
		this.onImageLoadHandler = function(e)
		{
			if (self.countLoadedGraphics == 0)
			{
				...
				self.dispatchEvent(FWDRDMData.PRELOADER_LOAD_DONE);
			}
			...
			{
				self.dispatchEvent(FWDRDMData.LOAD_DONE);
			}
		};
		...
		this.onImageLoadErrorHandler = function(e)
		{
			...
			self.dispatchEvent(FWDRDMData.LOAD_ERROR, err);
		};
		...
		this.checkForAttribute = function(e, attr)
		{
			...
			if (!test)
			{
				self.dispatchEvent(FWDRDMData.LOAD_ERROR, {text:"Element  with attribute <font color='#FFFFFF'>" + attr + "</font> is not defined."});
				return;
			}
	...
	/* set prototype */
	FWDRDMData.setPrototype = function()
	{
		FWDRDMData.prototype = new FWDEventDispatcher();
	};

	FWDRDMData.prototype = null;
	FWDRDMData.PRELOADER_LOAD_DONE = "onPreloaderLoadDone";
	FWDRDMData.LOAD_DONE = "onLoadDone";
	FWDRDMData.LOAD_ERROR = "onLoadError";

	window.FWDRDMData = FWDRDMData;
}(window));/* thumb */
(function(window)
{
	var FWDRDMThumb = function(id, data, parent)
	{
		var self = this;
		var prototype = FWDRDMThumb.prototype;
		...
		this.thumbSpace = parent.thumbSpace;
		
		this.thumbWidth = parent.thumbWidth;
		this.thumbHeight = parent.thumbHeight;
		
		this.thumbMinScale = parent.thumbMinScale;
		...
		this.setupScreen = function()
		{
			if (data.showThumbnailsHtmlContent || !data.transparentImages)
			{
				...
				self.addChild(self.borderDO);
				self.addChild(self.bgDO);
				
				if (self.borderSize == 0)
				{
					self.borderDO.setWidth(self.thumbWidth - 2);
					self.borderDO.setHeight(self.thumbHeight - 2);
					
					self.borderDO.setX(1);
					self.borderDO.setY(1);
					
					self.bgDO.setWidth(self.thumbWidth - self.borderSize * 2 - 2);
					self.bgDO.setHeight(self.thumbHeight - self.borderSize * 2 - 2);
					
					self.bgDO.setX(self.borderSize + 1);
					self.bgDO.setY(self.borderSize + 1);
				}
				else
				{
			...
			self.addChild(self.imageHolderDO);
			...
			if (self.showBoxShadow && !self.showRefl)
			{
			...
			if (FWDUtils.hasTransform2d)
			{
				switch (parent.thumbsAlignment)
				{
					case "top":
						self.setTransformOrigin("0%");
						self.setY((self.thumbHeight - self.thumbMinHeight)/2);
						break;
					case "bottom":
			...
			if (self.isMobile)
			{
				...
				{
					self.screen.addEventListener("MSPointerUp", self.onMouseTouchHandler);
				}
				else
				{
					self.screen.addEventListener("touchend", self.onMouseTouchHandler);
				}
			}
			else
			{
				if (self.showTooltip)
				{
					clearTimeout(self.tooltipTimeoutId);
					self.tooltipTimeoutId = setTimeout(self.setupTooltip, 10);
				}
			}
		};
		...
		this.setupTooltip = function()
		{
			...
			self.tooltipDO = new FWDTooltip(
			{
				skinPath:data.propsObj.skinPath + "/tooltip-skin",
				tooltipMaxWidth:300,
				text:self.curDataListAr[self.id].thumbText,
				btnMode:!data.showThumbnailsHtmlContent || !data.enableHtmlContent
			});
			...
			if (!self.dynTooltip)
			{
				var newY = -(self.thumbHeight * self.thumbScale - self.thumbHeight)/2 + parent.posY + parent.yOffset - 5;
				var ddy = 0;
				...
				if (!FWDUtils.hasTransform2d)
				{
					...
					{
						case "top":
							ddy = self.thumbHeight/2 - self.thumbHeight * self.thumbScale/2 + self.thumbMinHeight/2;
							break;
						case "bottom":
							ddy = self.thumbHeight/2 + self.thumbHeight * self.thumbScale/2 - self.thumbMinHeight/2;
							break;
						default:
							ddy = self.thumbHeight/2;
					}
				}
				else
				{
					...
					{
						case "top":
							ddy = -(self.thumbHeight * self.thumbScale - self.thumbMinHeight)/2;
							break;
						case "bottom":
							ddy = (self.thumbHeight * self.thumbScale - self.thumbMinHeight)/2;
							break;
				...
				self.tooltipDO.setX((parent.posX + self.thumbWidth/2) + self.newX);
				self.tooltipDO.setY(newY);
			}
			
			if (self.screen.addEventListener)
			{
				parent.parent.mainDO.screen.addEventListener("mousemove", self.onMouseMoveHandler);
			}
			else
			{
				parent.parent.mainDO.screen.attachEvent("onmousemove", self.onMouseMoveHandler);
			}
		};
		...
		this.addReflection = function()
		{
			...
			self.reflCanvasDO.screen.height = self.reflHeight;
			...
			var gradient = context.createLinearGradient(0, 0, 0, self.reflHeight);
			
			gradient.addColorStop(1, "rgba(255, 255, 255, 1.0)");
			gradient.addColorStop(0, "rgba(255, 255, 255, " + (1-self.reflAlpha) + ")");

			context.fillStyle = gradient;
			context.fillRect(0, 0, self.thumbWidth, self.reflHeight + 2);
			
			self.setWidth(self.thumbWidth);
			self.reflCanvasDO.setY(self.thumbHeight + self.reflDist);
		...
		this.hideTooltip = function()
		{
			...
			{
				TweenMax.to(self.tooltipDO, .4, {alpha:0, ease:Expo.easeOut, onComplete:self.posTooltip});
			}
		...
		this.onMouseMoveHandler = function(e)
		{
			...
			var vmc = FWDUtils.getViewportMouseCoordinates(e);
			...
			var dx = self.thumbWidth * self.thumbScale/2 - (parent.posX + self.thumbWidth/2) - self.newX + vmc.screenX - containerX;
			var dy = (self.thumbHeight * self.thumbScale - self.thumbHeight)/2 - parent.posY - parent.yOffset + vmc.screenY - containerY;
			...
			if ((dx >= 0) && (dx <= self.thumbWidth * self.thumbScale) && (dy >= -1) && (dy <= self.thumbHeight * self.thumbScale + 1))
			{
				if (!self.isOver)
				{
					self.isOver = true;
					...
					{
						TweenMax.to(self.tooltipDO, .4, {alpha:1, ease:Expo.easeOut});
					}
				...
				{
					newX = vmc.screenX - containerX - dx + self.thumbWidth * self.thumbScale/2;
					newY = vmc.screenY - containerY - dy - 5;
					...
					{
						TweenMax.to(self.tooltipDO, .3, {x:newX, y:newY});
					}	
				}
			}
			else
		...
		this.showThumb = function()
		{
			if (!FWDUtils.hasTransform2d)
			{
				var scaleW = self.thumbWidth * self.thumbScale;
				var scaleH = self.thumbHeight * self.thumbScale;
				var borderScale = self.borderSize * self.thumbScale;
				...
				self.imageHolderDO.setX(scaleW/2);
				self.imageHolderDO.setY(scaleH/2);
				...
				if (data.showThumbnailsHtmlContent)
				{
					if (self.htmlContentDO)
					{
						...
						self.htmlContentDO.setX(-imgW/2);
						self.htmlContentDO.setY(-imgH/2);
				...
				else
				{
					if (self.imageDO)
					{
						...
						self.imageDO.setX(-imgW/2);
						self.imageDO.setY(-imgH/2);
						
						TweenMax.to(self.imageDO, .8, {x:0, y:0, ease:Expo.easeInOut});
					}
				}
				
				clearTimeout(self.introTimeoutId);
				self.introTimeoutId = setTimeout(self.introReady, 800);
			}
			else
			{
				...
				self.imageHolderDO.setX(self.thumbWidth/2);
				self.imageHolderDO.setY(self.thumbHeight/2);
		...
		this.showThumbIntro = function(scale)
		{
			self.thumbScale = scale;

			if (!FWDUtils.hasTransform2d)
			{
				var scaleW = self.thumbWidth * scale;
				var scaleH = self.thumbHeight * scale;
				var borderScale = self.borderSize * scale;
				...
				self.setX(self.newX + (self.thumbWidth - scaleW)/2);
				self.setY(-self.thumbMinHeight/2);
			}
			else
			{
				self.setScale2(self.thumbScale);
			...
			TweenMax.to(self, .8, {alpha:1, ease:Quart.easeOut});
		};
		...
		this.setScale = function(scale)
		{
			...
			if (!FWDUtils.hasTransform2d)
			{
				...
				if (self.borderDO)
				{
					TweenMax.to(self.borderDO, .2, {w:scaleW, h:scaleH, ease:Quart.easeOut});
				}
				
				if (self.bgDO)
				{
					TweenMax.to(self.bgDO, .2, {x:borderScale, y:borderScale, w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
				}
				
				switch (parent.thumbsAlignment)
				{
					case "top":
						TweenMax.to(self, .2, {x:self.newX + (self.thumbWidth - scaleW)/2, y:-self.thumbMinHeight/2, w:scaleW, h:scaleH, ease:Quart.easeOut});
						break;
					case "bottom":
						TweenMax.to(self, .2, {x:self.newX + (self.thumbWidth - scaleW)/2, y:-scaleH + self.thumbMinHeight/2, w:scaleW, h:scaleH, ease:Quart.easeOut});
						break;
					default:
						TweenMax.to(self, .2, {x:self.newX + (self.thumbWidth - scaleW)/2, y:-scaleH/2, w:scaleW, h:scaleH, ease:Quart.easeOut});
				}
				
				if (self.isReady)
				{
					TweenMax.to(self.imageHolderDO, .2, {x:borderScale, y:borderScale, w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});

					if (data.showThumbnailsHtmlContent)
					{
						if (self.htmlContentDO)
						{
							TweenMax.to(self.htmlContentDO, .2, {w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
						}
					}
					else
					{
						if (self.imageDO)
						{
							TweenMax.to(self.imageDO, .2, {w:scaleW - borderScale * 2, h:scaleH - borderScale * 2, ease:Quart.easeOut});
						}
					}
				}
				else if (self.thumbScale > data.thumbMinScale)
				{
			...
			{	
				TweenMax.to(self, .3, {x:self.newX, scale:self.thumbScale});
			}
		};
		
		this.update = function(tw, th, ms, st, dt, sr, rd)
		{
			self.thumbMinScale = ms;
			self.thumbMinWidth = self.thumbWidth * self.thumbMinScale;
			self.thumbMinHeight = self.thumbHeight * self.thumbMinScale;
			
			if (self.isMobile && (self.thumbWidth != tw))
			{
				self.thumbWidth = tw;
				self.thumbHeight = th;
			...
			if (!self.isMobile && !self.tooltipDO && self.showTooltip)
			{
				clearTimeout(self.tooltipTimeoutId);
				self.tooltipTimeoutId = setTimeout(self.setupTooltip, 10);
			}
		...
		this.hide = function(del)
		{
			if (!FWDUtils.hasTransform2d)
			{
				...
				TweenMax.to(self.imageHolderDO, .8, {x:scaleW/2, y:scaleH/2, w:0, h:0, delay:del, ease:Expo.easeInOut});
				
				if (data.showThumbnailsHtmlContent)
				{
					TweenMax.to(self.htmlContentDO, .8, {x:-imgW/2, y:-imgH/2, delay:del, ease:Expo.easeInOut});
				}
				else
				{
					TweenMax.to(self.imageDO, .8, {x:-imgW/2, y:-imgH/2, delay:del, ease:Expo.easeInOut});
				}
			}
			else
			{
				...
				TweenMax.to(self.imageHolderDO, .8, {x:self.thumbWidth/2, y:self.thumbHeight/2, w:0, h:0, delay:del, ease:Expo.easeInOut});
		...
		{
			self.dispatchEvent(FWDRDMThumb.CLICK, {id:self.id});
		};
		...
		this.destroy = function()
		{
			...
			if (self.isMobile)
			{
				if (self.hasPointerEvent)
				{
					self.screen.removeEventListener("MSPointerUp", self.onMouseTouchHandler);
				}
				else
				{
					self.screen.removeEventListener("touchend", self.onMouseTouchHandler);
				}
				
				parent.parent.mainDO.screen.removeEventListener("mousemove", self.onMouseMoveHandler);
			}
			else
			{
				...
				else
				{
					...
					parent.parent.mainDO.screen.detachEvent("onmousemove", self.onMouseMoveHandler);
				}
			}
			...
			if (self.tooltipDO)
			{
				TweenMax.killTweensOf(self.tooltipDO);
				self.tooltipDO.destroy();
				self.tooltipDO = null;
			}
	...
	/* set prototype */
	FWDRDMThumb.setPrototype = function()
	{
		FWDRDMThumb.prototype = new FWDDisplayObjectScale("div", "absolute", "visible");
	};

	FWDRDMThumb.CLICK = "click";

	FWDRDMThumb.prototype = null;
	window.FWDRDMThumb = FWDRDMThumb;
}(window));/* thumbs manager */
(function(window)
{
	var FWDRDMThumbsManager = function(data, parent)
	{
		var self = this;
		var prototype = FWDRDMThumbsManager.prototype;
		...
		this.thumbSpace = data.thumbSpace;
		this.sideSpace = data.sideSpace;
		
		this.thumbMinScale = data.thumbMinScale;
		this.thumbMinWidth = this.thumbWidth * this.thumbMinScale;
		this.thumbMinHeight = this.thumbHeight * this.thumbMinScale;
		
		this.maxDist = data.inflDist;
		...
		this.menuStyleTemp = data.menuStyle;
		...
		this.thumbsAlignment = data.thumbsAlignment;
		...
		this.init = function()
		{
			if (FWDUtils.hasTransform2d)
			{
				self.posY = (self.stageHeight - self.thumbHeight)/2;
			}
			else
			{
				self.posY = self.stageHeight/2;
			}
		...
		this.showCurrentCat = function(id)
		{
			...
			if (self.isMobile)
			{
				...
				if (self.stageWidth < self.thumbWidth)
				{
					self.thumbWidth = self.stageWidth - 20;
					self.thumbHeight = self.thumbWidth * (data.thumbHeight /  data.thumbWidth);
				}
				else
				{
			...
			var maxMenuDist = self.totalThumbs * self.thumbWidth * 2;

			self.setWidth(maxMenuDist);
			self.setHeight(parent.originalHeight);
			...
			if (!self.menuStyle)
			{
				var totalDist = self.totalThumbs * (self.thumbSpace + self.thumbMinWidth) - self.thumbSpace;
				
				if (totalDist < self.stageWidth)
				{
					var dmargin = (self.stageWidth - totalDist) / 2;
					self.mouseX = dmargin;
				}
				else
				{
					self.mouseX = self.thumbWidth/2;
				}
			}
		...
		this.hideCurrentCat = function()
		{
			...
			clearTimeout(self.autoplayStartId);
			clearTimeout(self.autoplayStopId);
			clearTimeout(self.tooltipHideId);
			...
			else
			{
				if (self.screen.addEventListener)
				{
					parent.mainDO.screen.removeEventListener("mousemove", self.onMouseMove);
					parent.mainDO.screen.removeEventListener("mouseover", self.removeAutoplay);
					parent.mainDO.screen.removeEventListener("mouseout", self.initAutoplay);
				}
				else
				{
					parent.mainDO.screen.detachEvent("onmousemove", self.onMouseMove);
					parent.mainDO.screen.detachEvent("onmouseover", self.removeAutoplay);
					parent.mainDO.screen.detachEvent("onmouseout", self.initAutoplay);
				}
			}
			
			if (self.hasPointerEvent)
			{
				parent.mainDO.screen.removeEventListener("MSPointerMove", self.onMouseMove);
			}
		...
		{	
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				TweenMax.to(thumb, .8, {alpha:0, ease:Expo.easeInOut});
				thumb.hide(0);
			}
			
			self.hideThumbsFinishedId = setTimeout(self.allThumbsAreTweened, 800);
		};
		...
		this.setupThumbs = function()
		{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				thumb = new FWDRDMThumb(i, self.data, self);
				
				self.thumbsAr.push(thumb);
				
				thumb.addListener(FWDRDMThumb.CLICK, self.onThumbClick);
			}
		};
		...
		this.thumbClickHandler = function()
		{
			...
			else
			{
				if (!data.showThumbnailsHtmlContent || !data.enableHtmlContent)
				{
					...
					self.hideTooltip();
					self.dispatchEvent(FWDRDMThumbsManager.THUMB_CLICK, {id:tempId});
				}
			}
		};
		...
		this.loadImages = function()
		{
			...
			self.countLoadedThumbsRight = self.curId;
		...
		this.onImageLoadErrorHandler = function(e)
		{
			...
			self.dispatchEvent(FWDRDMThumbsManager.LOAD_ERROR, {text : message});
		};
		...
		this.setupIntro = function()
		{
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				newX = i * (self.thumbMinWidth + self.thumbSpace);
				newScale = self.thumbMinScale;
				
				thumb.newX = newX;
				
				thumb.showThumbIntro(newScale);
			...
			if (totalDist < self.stageWidth)
			{
				self.posX = (self.stageWidth - totalDist)/2 - (self.thumbWidth - self.thumbMinWidth)/2;
			}
			else
			{
				self.posX = -(self.thumbWidth - self.thumbMinWidth)/2 + self.thumbSpace;
			}

			self.setX(self.posX);
			
			self.setIntroFinishedId = setTimeout(self.setIntroFinished, 800);
		};
		...
		this.setIntroFinished = function()
		{
			...
			self.dispatchEvent(FWDRDMThumbsManager.THUMBS_INTRO_FINISH);
			...
			else
			{
				if (self.screen.addEventListener)
				{
					parent.mainDO.screen.addEventListener("mousemove", self.onMouseMove);
				}
				else
				{
					parent.mainDO.screen.attachEvent("onmousemove", self.onMouseMove);
				}
			}
			
			if (self.hasPointerEvent)
			{
				parent.mainDO.screen.addEventListener("MSPointerMove", self.onMouseMove);
			}
		...
		{
			self.menuStyle = self.menuStyleTemp;
			
			if (self.screen.addEventListener)
			{
				parent.mainDO.screen.addEventListener("mouseover", self.removeAutoplay);
				parent.mainDO.screen.addEventListener("mouseout", self.initAutoplay);
			}
			else
			{
				parent.mainDO.screen.attachEvent("onmouseover", self.removeAutoplay);
				parent.mainDO.screen.attachEvent("onmouseout", self.initAutoplay);
			}
		...
		this.onMouseMove = function(e)
		{
			...
			self.mouseX = viewportMouseCoordinates.screenX - parent.rect.left;
			self.mouseY = viewportMouseCoordinates.screenY - parent.rect.top;
		...
		this.initAutoplay = function()
		{
			clearTimeout(self.autoplayStartId);
			clearTimeout(self.autoplayStopId);
			self.autoplayStartId = setTimeout(self.startAutoplay, 30);
		};
		...
		this.startAutoplay = function()
		{
			...
			else
			{
				...
				clearInterval(self.autoplayId);
				self.autoplayId = setInterval(self.updateAutoplay, 20);
			}
			
			clearTimeout(self.tooltipHideId);
			self.tooltipHideId = setTimeout(self.hideTooltip, 100);
			
			if (self.menuStyle)
			{
				clearInterval(self.updateId);
				self.updateId = setInterval(self.updateDockMenu, 20);
			}
		};
		...
		this.removeAutoplay = function()
		{
			...
			clearTimeout(self.autoplayStartId);
			clearTimeout(self.autoplayStopId);
			self.autoplayStopId = setTimeout(self.stopAutoplay, 30);
		};
		...
		this.updateAutoplay = function()
		{
			...
			var totalDist = (self.totalThumbs - 1) * self.thumbSpace;
			
			for (var i=0; i<self.totalThumbs; i++)
			{
				thumb = self.thumbsAr[i];
			
				totalDist += self.thumbWidth * thumb.thumbScale;
			}
			
			if (totalDist < self.stageWidth)
			{
				self.mouseX += (self.autoplaySpeed/20) * (12/self.totalThumbs) * (totalDist / 1000) * self.autoplayDir;
				
				var dmargin = (self.stageWidth - totalDist) / 2;
				
				if (self.mouseX < (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4)) + dmargin)
				{
					self.mouseX = (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4)) + dmargin;
					self.autoplayDir = 1;
				}
				
				if (self.mouseX > self.stageWidth - (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4)) - dmargin)
				{
					self.mouseX = self.stageWidth - (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4)) - dmargin;
					self.autoplayDir = -1;
				}
			}
			else
			{
				self.mouseX += (self.autoplaySpeed/20) * (12/self.totalThumbs) * ((self.stageWidth - self.thumbWidth) / 1000) * self.autoplayDir;
				
				if (self.mouseX < self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 2))
				{
					self.mouseX = self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4);
					self.autoplayDir = 1;
				}
				
				if (self.mouseX > self.stageWidth - (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4)))
				{
					self.mouseX = self.stageWidth - (self.thumbWidth / 2  - (self.stageWidth - self.thumbWidth) / ((self.totalThumbs - 1) * 4));
					self.autoplayDir = -1;
				}
			}
		...
		this.updateThumbsPos = function()
		{
			...
			if (totalDist < self.stageWidth)
			{
				self.posX = (self.stageWidth - totalDist) / 2 - self.thumbWidth / 2;
			}
			else
			{
				self.posX = -((self.mouseX - self.thumbWidth / 2)  / (self.stageWidth - self.thumbWidth)) * (totalDist - self.stageWidth) - self.thumbWidth / 2;
				
				self.posX = Math.min(self.posX, self.sideSpace - self.thumbWidth / 2);
				self.posX = Math.max(self.posX, self.stageWidth - totalDist - self.sideSpace - self.thumbWidth / 2);
			}
			...
			for (var i=0; i<self.totalThumbs; i++)
			{
				...
				var thDist = Math.abs((thumb.newX + self.posX + self.thumbWidth / 2) - self.mouseX);

				if (self.menuStyle)
				{
					if ((thDist < self.maxDist) && self.mouseOver)
					{
						var scale = 1 + (1 - (thDist / self.maxDist)) * (self.thumbWidth/self.thumbMinWidth - 1);
						
						newScale[i] = (self.thumbMinScale * scale);
					}
					else
					{
						newScale[i] = self.thumbMinScale;
					}
				}
				else
				{
					if (thDist < self.maxDist)
					{
						var scale = 1 + (1 - (thDist / self.maxDist)) * (self.thumbWidth/self.thumbMinWidth - 1);
			...
			thumb = self.thumbsAr[0];
			thumb.thumbScale = newScale[0];
			thumb.newX = self.thumbWidth * thumb.thumbScale / 2;
			
			for (var i=1; i<self.totalThumbs; i++)
			{
				thumb = self.thumbsAr[i];
				var thumb2 = self.thumbsAr[i-1];
				
				thumb.thumbScale = newScale[i];
				
				var dist = (thumb.newX - self.thumbWidth * thumb.thumbScale / 2) - (thumb2.newX + self.thumbWidth * thumb2.thumbScale / 2);
				
				thumb.newX = thumb.newX + self.thumbSpace - dist;
			}
		};
		...
		this.updateThumbs = function()
		{
			...
			TweenMax.to(self, .3, {x:self.posX});
		};
		...
		this.mobileDragMoveTest = function(e)
		{
			...
			self.mX = viewportMouseCoordinates.screenX;
			self.mY = viewportMouseCoordinates.screenY;
		...
		this.mobileDragMoveHandler = function(e)
		{
			...
			var dx = (self.lastPressedX - self.mX) / 5;
			...
			if (totalDist < self.stageWidth)
			{
				self.mouseX = self.lastMouseX + dx * (12/self.totalThumbs) * (totalDist / 400);
				
				var dmargin = (self.stageWidth - totalDist) / 2;
				
				if (self.mouseX < self.thumbWidth/2 + dmargin)
				{
					self.mouseX = self.thumbWidth/2 + dmargin;
				}
				
				if (self.mouseX > self.stageWidth - self.thumbWidth/2 - dmargin)
				{
					self.mouseX = self.stageWidth - self.thumbWidth/2 - dmargin;
				}
			}
			else
			{
				self.mouseX = self.lastMouseX + dx * (12/self.totalThumbs) * ((self.stageWidth - self.thumbWidth) / 400);
				
				if (self.mouseX < self.thumbWidth/2)
				{
					self.mouseX = self.thumbWidth/2;
				}
				
				if (self.mouseX > self.stageWidth - self.thumbWidth/2)
				{
					self.mouseX = self.stageWidth - self.thumbWidth/2;
				}
			}
		...
		{
			self.thumbSpace = e.thumbSpace;
			self.autoplaySpeed = e.autoplaySpeed;
			self.maxDist = e.inflDist;
			self.sideSpace = e.sideSpace;
			self.yOffset = e.yOffset;
			self.menuStyleTemp = e.menuStyle;
			self.menuStyle = self.menuStyleTemp;
			self.showTooltip = e.showTooltip;
			self.dynTooltip = e.dynTooltip;
			self.showRefl = e.showRefl;
			self.reflDist = e.reflDist;
			
			self.thumbMinScale = e.thumbMinScale;
			self.thumbMinWidth = self.thumbWidth * self.thumbMinScale;
			self.thumbMinHeight = self.thumbHeight * self.thumbMinScale;
			
			self.thumbsAlignment = self.alignmentsAr[e.thumbsAlignment];
			
			self.updateThumbsScaleTooltip();

			self.setY(self.posY + self.yOffset);
		...
		this.updateThumbsScaleTooltip = function()
		{
			for (var i=0; i<self.totalThumbs; i++)
			{
				self.thumbsAr[i].update(self.thumbWidth, self.thumbHeight, self.thumbMinScale, self.showTooltip, self.dynTooltip, self.showRefl, self.reflDist);
			}
		};
		...
		{			
			...
			prototype.destroy();
			prototype = null;
			FWDRDMThumbsManager.prototype = null;
		};
	...
	/* set prototype */
	FWDRDMThumbsManager.setPrototype = function()
	{
		FWDRDMThumbsManager.prototype = new FWDDisplayObject("div", "absolute", "visible");
	};
	
	FWDRDMThumbsManager.THUMB_CLICK = "onThumbClick";
	FWDRDMThumbsManager.LOAD_ERROR = "onLoadError";
	FWDRDMThumbsManager.THUMBS_INTRO_FINISH = "onThumbsIntroFinish";

	window.FWDRDMThumbsManager = FWDRDMThumbsManager;

}(window));/* DockMenu */
(function(window)
{
	var FWDRoyalDockMenu = function(props)
	{
		var self = this;
		...
		this.init = function()
		{
			...
			if (!self.propsObj.dockMenuHolderDivId)
			{
				alert("Property dockMenuHolderDivId is not defined in the FWDRoyalDockMenu object constructor!");
				return;
			}
			
			if (!FWDUtils.getChildById(self.propsObj.dockMenuHolderDivId))
			{
				alert("FWDRoyalDockMenu holder div is not found, please make sure that the div exists and the id is correct! " + self.propsObj.dockMenuHolderDivId);
				return;
			}
			
			if (!self.propsObj.dockMenuWidth)
			{
			...
			if (!self.propsObj.dockMenuHeight)
			{
			...
			self.stageContainer = FWDUtils.getChildById(self.propsObj.dockMenuHolderDivId);
			
			self.autoScale = self.propsObj.autoScale == "yes" ? true : false;
			
			self.originalWidth = self.propsObj.dockMenuWidth;
			self.originalHeight = self.propsObj.dockMenuHeight;
		...
		this.setupMainDO = function()
		{
			...
			if (self.displayType == FWDRoyalDockMenu.FLUID_WIDTH)
			{	
		...
		this.resizeHandler = function()
		{
			...
			if (self.displayType == FWDRoyalDockMenu.FLUID_WIDTH)
			{
				...
				self.mainDO.setX(self.pageXOffset);
				// Setting to 0 because it was causing problems
				// self.mainDO.setY(Math.round(self.stageContainer.getBoundingClientRect().top + self.pageYOffset));
				self.mainDO.setY(0);
			}
			else
			...
			if (self.preloaderLoaded)
			{
				if (self.propsObj.backgroundImagePath)
				{
					if (self.displayType == FWDRoyalDockMenu.RESPONSIVE)
					{
		...
		this.setupData = function()
		{
			...
			self.data = new FWDRDMData(self.propsObj);
			self.data.addListener(FWDRDMData.PRELOADER_LOAD_DONE, self.onPreloaderLoadDone);
			self.data.addListener(FWDRDMData.LOAD_ERROR, self.dataLoadError);
			self.data.addListener(FWDRDMData.LOAD_DONE, self.dataLoadComplete);
		};
		...
		{
			self.dispatchEvent(FWDRoyalDockMenu.DATA_LOADED);
		...
		this.setupPreloader = function()
		{
			...
			self.preloaderDO = new FWDPreloader(self.data.mainPreloaderImg, 57, 28, 21, 50);
		...
		this.setupThumbsManager = function()
		{
			...
			self.thumbsManagerDO = new FWDRDMThumbsManager(self.data, self);
			self.thumbsManagerDO.addListener(FWDRDMThumbsManager.THUMB_CLICK, self.onThumbsManagerThumbClick);
			self.thumbsManagerDO.addListener(FWDRDMThumbsManager.LOAD_ERROR, self.onThumbsManagerLoadError);
			self.thumbsManagerDO.addListener(FWDRDMThumbsManager.THUMBS_INTRO_FINISH, self.onThumbsManagerIntroFinish);
		...
		{
			self.enableAll();
			self.dispatchEvent(FWDRoyalDockMenu.INTRO_FINISH);
			
			self.isAPIReady = true;
			self.dispatchEvent(FWDRoyalDockMenu.IS_API_READY);
		};
		...
		this.setupComboBox = function()
		{
			...
			self.comboBoxDO.addListener(FWDComboBox.OPEN, self.onComboboxOpenHandler);
		...
		this.onComboboxButtonPressHandler = function(e)
		{
			if (self.thumbsManagerDO.allowToSwitchCat)
			{
				...
				self.dispatchEvent(FWDRoyalDockMenu.INTRO_START);
		...
		{
			self.thumbsManagerDO.hideTooltip();
		};
		...
		this.switchCategory = function(id)
		{
			...
			{
				if (self.thumbsManagerDO && self.thumbsManagerDO.allowToSwitchCat)
				{
					self.disableAll();
		...
		this.setupLightBox = function()
		{
			...
			self.lightboxDO.addListener(FWDLightBox.HIDE_COMPLETE, self.onLightboxHideComplete);
		};
		
		this.onLightboxHideComplete = function()
		{
			self.thumbsManagerDO.enableAutoplay();
		};
	...
	FWDRoyalDockMenu.RESIZE = "resize";
	FWDRoyalDockMenu.LIGHTBOX = "lightbox";
	FWDRoyalDockMenu.RESPONSIVE = "responsive";
	FWDRoyalDockMenu.FLUID_WIDTH = "fluidwidth";
	
	FWDRoyalDockMenu.INTRO_START = "onIntroStart";
	FWDRoyalDockMenu.INTRO_FINISH = "onIntroFinish";
	FWDRoyalDockMenu.DATA_LOADED = "onDataLoaded";
	FWDRoyalDockMenu.IS_API_READY = "isAPIReady";

	
	window.FWDRoyalDockMenu = FWDRoyalDockMenu;
...
(function(window)
{
	var FWDTooltip = function(propsObj)
	{
		var self = this;
		var prototype = FWDTooltip.prototype;
		...
		{
			self.tooltipMainDO = new FWDDisplayObject("div");
			self.addChild(self.tooltipMainDO);
			
			self.tooltipHolderDO = new FWDDisplayObject("div");
			self.tooltipMainDO.addChild(self.tooltipHolderDO);
			
			self.tooltipHolderDO.setWidth(1000);
			
			self.tooltipTextDO = new FWDSimpleDisplayObject("div");
		...
		{
			//center
			self.tooltipCenterDO = new FWDSimpleDisplayObject("img");
			...
			// top
			self.tooltipTopDO = new FWDSimpleDisplayObject("img");
			...
			//left
			self.tooltipLeftDO = new FWDSimpleDisplayObject("img");
			...
			//right
			self.tooltipRightDO = new FWDSimpleDisplayObject("img");
			...
			//bottom
			self.tooltipBottomDO = new FWDSimpleDisplayObject("img");
		...
		{
			self.tooltipPointerBottomDO = new FWDSimpleDisplayObject("img");
		...
		{
			self.tooltipPointerTopDO = new FWDSimpleDisplayObject("img");
			...
			self.tooltipOverDO = new FWDDisplayObject("div");
	...
	/* set prototype */
	FWDTooltip.setPrototype = function()
	{
		FWDTooltip.prototype = new FWDDisplayObject("div", "absolute", "visible");
	};

	FWDTooltip.CHANGE = "onChange";

	FWDTooltip.prototype = null;
	window.FWDTooltip = FWDTooltip;
}(window));//FWDUtils
(function (window){
	...
	FWDUtils.hasCanvas = function(){
		return Boolean(parent.document.createElement("canvas"));
	};
	...
	FWDUtils.getScrollOffsets = function(){
		//all browsers
		if(parent.window.pageXOffset != null) return{x:parent.window.pageXOffset, y:parent.window.pageYOffset};
		
		//ie7/ie8
		if(parent.document.compatMode == "CSS1Compat"){
			return({x:parent.document.documentElement.scrollLeft, y:parent.document.documentElement.scrollTop});
		}
	};
	
	FWDUtils.getViewportSize = function(){
		if(FWDUtils.hasPointerEvent && navigator.msMaxTouchPoints > 1){
			return {w:parent.document.documentElement.clientWidth || parent.window.innerWidth, h:parent.document.documentElement.clientHeight || parent.window.innerHeight};
		}
		
		if(FWDUtils.isMobile) return {w:parent.window.innerWidth, h:parent.window.innerHeight};
		return {w:parent.document.documentElement.clientWidth || parent.window.innerWidth, h:parent.document.documentElement.clientHeight || parent.window.innerHeight};
	};
	...
	FWDUtils.hasPointerEvent = (function(){
		return Boolean(parent.window.navigator.msPointerEnabled);
	}());
