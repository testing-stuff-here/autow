...
(function(global, factory) {
	...
	var isClient, isJsdom, hasConsole, environment__magic, namespaces, svg,
		vendors;
	...
	hasConsole = typeof console !== "undefined" && typeof console.warn ===
		"function" && typeof console.warn.apply === "function";
	...
	var createElement, matches, dom__div, methodNames, unprefixed, prefixed,
		dom__i, j, makeFunction;
	...
	if (!isClient) {
		...
		while (dom__i-- && !matches) {
			...
			if (dom__div[unprefixed]) {
				...
				j = vendors.length;
				while (j--) {
					prefixed = vendors[dom__i] + unprefixed.substr(0, 1).toUpperCase() +
	...
	function utils_object__extend(target) {
		for (var _len = arguments.length, sources = Array(_len > 1 ? _len - 1 : 0),
	...
	if (hasConsole) {
		(function() {
			var welcomeIntro = ["%cRactive.js %c0.7.3 %cin debug mode, %cmore...",
			...
			welcome = function() {
				...
				console[hasGroup ? "groupCollapsed" : "log"].apply(console,
					welcomeIntro);
			...
			printWarning = function(message, args) {
				...
				if (typeof args[args.length - 1] === "object") {
					...
					if (ractive) {
						...
						var node = undefined;
						if (node = options.node || ractive.fragment && ractive.fragment.rendered &&
							ractive.find("*")) {
							args.push(node);
						}
					}
				...
				console.warn.apply(console, ["%cRactive.js: %c" + message,
	...
	var interpolate = function(from, to, ractive, type) {
		...
		return static_interpolators.number(from, to) || static_interpolators.array(
			from, to) || static_interpolators.object(from, to) || snap(to);
	};
	...
	Keypath.prototype = {
		...
		replace: function(oldKeypath, newKeypath) {
			...
			if (this.startsWith(oldKeypath)) {
				return newKeypath === null ? newKeypath : getKeypath(this.str.replace(
					oldKeypath.str + ".", newKeypath.str + "."));
			}
		},
		...
		startsWith: function(keypath) {
			...
			return keypath && this.str.substr(0, keypath.str.length + 1) === keypath
				.str + ".";
		},
		...
		wildcardMatches: function() {
			return this._wildcardMatches || (this._wildcardMatches =
				utils_getPotentialWildcardMatches(this.str));
		}
	};
	...
	function assignNewKeypath(target, property, oldKeypath, newKeypath) {
		...
		if (existingKeypath && (existingKeypath.equalsOrStartsWith(newKeypath) ||
				!existingKeypath.equalsOrStartsWith(oldKeypath))) {
			return;
		}

		target[property] = existingKeypath ? existingKeypath.replace(oldKeypath,
	...
	function getMatchingKeypaths(ractive, keypath) {
		...
		function expand(matchingKeypaths, keypath) {
			...
			if (keypath.isRoot) {
				keys = [].concat(Object.keys(ractive.viewmodel.data), Object.keys(
					ractive.viewmodel.mappings), Object.keys(ractive.viewmodel.computations));
			} else {
	...
	function add(root, keypath, d) {
		...
		if (/\*/.test(keypath)) {
			...
			getMatchingKeypaths(root, getKeypath(normalise(keypath))).forEach(
				function(keypath) {
	...
	if (typeof window !== "undefined" && window.performance && typeof window.performance
		.now === "function") {
		getTime = function() {
		...
		if (!ractive[this.method] && this.deprecate && call(this.deprecate.deprecated)) {
			if (this.deprecate.message) {
				...
				warnIfDebug(
					...
					this.deprecate.deprecated, this.deprecate.replacement);
			}
		}
		...
		_Promise = function(callback) {
			...
			makeResolver = function(newState) {
				return function(value) {
					...
					dispatchHandlers = makeDispatcher(state === FULFILLED ?
						fulfilledHandlers : rejectedHandlers, result);
	...
	function resolveAmbiguousReference(ractive, ref, fragment, isParentLookup) {
		...
		while (fragment) {
			...
			if (parentValue && (typeof parentValue === "object" || typeof parentValue ===
					"function") && key in parentValue) {
		...
		if (ractive.parent && !ractive.isolated) {
			...
			if (parentKeypath = resolveAmbiguousReference(ractive.parent, key,
					fragment, true)) {
	...
	function createMappingIfNecessary(ractive, key) {
		...
		if (!ractive.parent || ractive.isolated || isRootProperty(ractive.viewmodel,
				key)) {
			return;
		}
		...
		if (parentKeypath = resolveAmbiguousReference(ractive.parent, key, ractive
				.component.parentFragment, true)) {
			ractive.viewmodel.map(key, {
				origin: ractive.parent.viewmodel,
				keypath: parentKeypath
			});
	...
	function isRootProperty(viewmodel, key) {
		...
		return key === "" || key in viewmodel.data || key in viewmodel.computations ||
	...
	runloop = {
		start: function(instance, returnPromise) {
			...
			batch = {
				previousBatch: batch,
				transitionManager: new global_TransitionManager(fulfilPromise, batch &&
					batch.transitionManager),
				views: [],
				tasks: [],
				ractives: [],
				instance: instance
	...
	var Animation = function(options) {
		...
		this.interpolator = shared_interpolate(this.from, this.to, this.root, this
			.interpolator);
		this.running = true;
	...
	Animation.prototype = {
		tick: function() {
			...
			if (this.running) {
				...
				t = this.easing ? this.easing(elapsed / this.duration) : elapsed / this
					.duration;
	...
	function Ractive$animate(keypath, to, options) {
		var promise, fulfilPromise, k, animation, animations, easing, duration,
			step, complete, makeValueCollector, currentValues, collectValue, dummy,
			dummyOptions;
	...
	var makeQuery_cancel = function() {
		...
		liveQueries = this._root[this._isComponentQuery ? "liveComponentQueries" :
			"liveQueries"];
	...
	var sortByItemPosition = function(a, b) {
		var ancestryA, ancestryB, oldestA, oldestB, mutualAncestor, indexA, indexB,
	...
	var sort = function() {
		this.sort(this._isComponentQuery ? sortByItemPosition :
	...
	var remove = function(nodeOrComponent) {
		var index = this.indexOf(this._isComponentQuery ? nodeOrComponent.instance :
			nodeOrComponent);
	...
	function Ractive$findContainer(selector) {
		if (this.container) {
			if (this.container.component && this.container.component.name ===
				selector) {
	...
	function fireEventAs(ractive, eventNames, event, args) {
		...
		if (ractive.parent && bubble) {
			...
			if (initialFire && ractive.component) {
				var fullName = ractive.component.name + "." + eventNames[eventNames.length -
					1];
	...
	PatternObserver = function(ractive, keypath, callback, options) {
		...
		this.keypath = keypath;
		this.regex = new RegExp("^" + keypath.str.replace(/\./g, "\\.").replace(
			/\*/g, "([^\\.]+)") + "$");
		this.values = {};
	...
	function getObserverFacade(ractive, keypath, callback, options) {
		...
		if (keypath.isPattern) {
			observer = new observe_PatternObserver(ractive, keypath, callback,
				options);
		...
		ractive.viewmodel.register(keypath, observer, isPatternObserver ?
			"patternObservers" : "observers");
	...
	function Ractive$observe(keypath, callback, options) {
		...
		while (i--) {
			...
			if (keypath) {
				observers.push(observe_getObserverFacade(this, keypath, callback,
					options));
			}
		}
	...
	var makeArrayMethod = function(methodName) {
		return function(keypath) {
			...
			if (!isArray(array)) {
				throw new Error("Called ractive." + methodName + "('" + keypath.str +
					"'), but '" + keypath.str + "' does not refer to an array");
			}
	...
	function transformCss(css, id) {
		...
		if (dataRvcGuidPattern.test(css)) {
			...
			transformed = css.replace(commentsPattern, "").replace(selectorsPattern,
				function(match, $1) {
	...
	var cssConfigurator = {
		...
		extend: function(Parent, proto, options) {
			if (options.css) {
				var id = css_css__uid++;
				var styles = options.noCssTransform ? options.css : transform(options.css,
					id);
	...
	function validate(data) {
		...
		if (data && data.constructor !== Object) {
			if (typeof data === "function") {} else if (typeof data !== "object") {
				fatal("data option must be an object or a function, `" + data +
					"` is not valid");
			} else {
				warnIfDebug(
	...
	function custom_data__combine(parentValue, childValue) {
		...
		return function() {
			...
			var parent = parentIsFn ? callDataFunction(parentValue, this) :
				parentValue;
	...
	Parser.prototype = {
		...
		error: function(message) {
			...
			var error = new ParseError("" + message + " at line " + lineNum +
				" character " + columnNum + ":\n" + annotation);
	...
	var makeQuotedStringMatcher = function(okQuote) {
		return function(parser) {
			...
			while (!done) {
				next = parser.matchPattern(stringMiddlePattern) || parser.matchPattern(
					escapeSequencePattern) || parser.matchString(okQuote);
				if (next) {
					if (next === "\"") {
						literal += "\\\"";
					} else if (next === "\\'") {
	...
	function readLiteral(parser) {
		return literal_readNumberLiteral(parser) || literal_readBooleanLiteral(
				parser) || readStringLiteral(parser) || readObjectLiteral(parser) ||
			readArrayLiteral(parser) || readRegexpLiteral(parser);
	}
	...
	var readPrimary = function(parser) {
		return primary_readLiteral(parser) || primary_readReference(parser) ||
			primary_readBracketedExpression(parser);
	};
	...
	function flattenExpression(expression) {
		...
		function stringify(node) {
			switch (node.t) {
				...
				case INFIX_OPERATOR:
					return stringify(node.o[0]) + (node.s.substr(0, 2) === "in" ? " " +
						node.s + " " : node.s) + stringify(node.o[1]);
				...
				case CONDITIONAL:
					return stringify(node.o[0]) + "?" + stringify(node.o[1]) + ":" +
						stringify(node.o[2]);
	...
	function refineExpression(expression, mustache) {
		...
		if (expression) {
			...
			if (expression.t === REFERENCE) {
				...
				if (expression.t === NUMBER_LITERAL && arrayMemberPattern.test(
						expression.v)) {
	...
	function readExpressionOrReference(parser, expectedFollowers) {
		...
		for (i = 0; i < expectedFollowers.length; i += 1) {
			if (parser.remaining().substr(0, expectedFollowers[i].length) ===
				expectedFollowers[i]) {
				return expression;
			}
		}
	...
	var indexRefPattern = /^\s*:\s*([a-zA-Z_$][a-zA-Z_$0-9]*)/,
		...
		handlebarsBlockPattern = new RegExp("^(" + Object.keys(handlebarsBlockCodes)
			.join("|") + ")\\b");

	function readSection(parser, tag) {
		var start, expression, section, child, children, hasElse, block,
			unlessBlock, conditions, closed, i, expectedClose;
		...
		do {
			if (child = section_readClosing(parser, tag)) {
				...
				unlessBlock.f.push({
					t: SECTION,
					n: SECTION_IF,
					x: utils_flattenExpression(mustache_readSection__combine(conditions.concat(
						child.x))),
					f: children = []
				});
	...
	function mustache_readSection__combine(expressions) {
		...
		return {
			t: INFIX_OPERATOR,
			s: "&&",
			o: [parensIfNecessary(expressions[0]), parensIfNecessary(
				mustache_readSection__combine(expressions.slice(1)))]
		};
	}
	...
	var booleanAttributes, voidElementNames, htmlEntities, controlCharacters,
		entityPattern, lessThan, greaterThan, amp;
	...
	controlCharacters = [8364, 129, 8218, 402, 8222, 8230, 8224, 8225, 710, 8240,
		352, 8249, 338, 141, 381, 143, 144, 8216, 8217, 8220, 8221, 8226, 8211,
		8212, 732, 8482, 353, 8250, 339, 157, 382, 376
	];
	entityPattern = new RegExp("&(#?(?:x[\\w\\d]+|\\d+|" + Object.keys(
		htmlEntities).join("|") + "));?", "g");
	...
	function escapeHtml(str) {
		return str.replace(amp, "&amp;").replace(lessThan, "&lt;").replace(
			greaterThan, "&gt;");
	}
	...
	var stripStandalones = function(items) {
		...
		for (i = 1; i < items.length; i += 1) {
			...
			if (isSection(current) && isString(backOne)) {
				if (trailingLinebreak.test(backOne) && isString(current.f[0]) &&
			...
			if (isString(current) && isSection(backOne)) {
				...
				if (isString(lastSectionItem) && trailingLinebreak.test(lastSectionItem) &&
	...
	function cleanup(items, stripComments, preserveWhitespace,
		removeLeadingWhitespace, removeTrailingWhitespace) {
		var i, item, previousItem, nextItem, preserveWhitespaceInsideFragment,
			removeLeadingWhitespaceInsideFragment,
			removeTrailingWhitespaceInsideFragment, key;
		...
		trimWhitespace(items, removeLeadingWhitespace ?
			utils_cleanup__leadingWhitespace : null, removeTrailingWhitespace ?
		...
		while (i--) {
			...
			// Recurse
			if (item.f) {
				var isPreserveWhitespaceElement = item.t === ELEMENT &&
					preserveWhitespaceElements.test(item.e);
				preserveWhitespaceInsideFragment = preserveWhitespace ||
					isPreserveWhitespaceElement;
				...
				if (!preserveWhitespaceInsideFragment) {
					...
					if (!previousItem || typeof previousItem === "string" &&
						trailingWhitespace.test(previousItem)) {
						removeLeadingWhitespaceInsideFragment = true;
					}

					// and vice versa
					if (!nextItem || typeof nextItem === "string" &&
						utils_cleanup__leadingWhitespace.test(nextItem)) {
						removeTrailingWhitespaceInsideFragment = true;
					}
				}

				cleanup(item.f, stripComments, preserveWhitespaceInsideFragment,
					removeLeadingWhitespaceInsideFragment,
			...
			if (item.l) {
				cleanup(item.l.f, stripComments, preserveWhitespace,
					removeLeadingWhitespaceInsideFragment,
					removeTrailingWhitespaceInsideFragment);
			...
			if (item.a) {
				for (key in item.a) {
					if (item.a.hasOwnProperty(key) && typeof item.a[key] !== "string") {
						cleanup(item.a[key], stripComments, preserveWhitespace,
							removeLeadingWhitespaceInsideFragment,
							removeTrailingWhitespaceInsideFragment);
					}
				}
			...
			if (item.m) {
				cleanup(item.m, stripComments, preserveWhitespace,
					removeLeadingWhitespaceInsideFragment,
					removeTrailingWhitespaceInsideFragment);
			}
				...
				for (key in item.v) {
					if (item.v.hasOwnProperty(key)) {
						...
						if (isArray(item.v[key].n)) {
							cleanup(item.v[key].n, stripComments, preserveWhitespace,
								removeLeadingWhitespaceInsideFragment,
								removeTrailingWhitespaceInsideFragment);
						}
						...
						if (isArray(item.v[key].d)) {
							cleanup(item.v[key].d, stripComments, preserveWhitespace,
								removeLeadingWhitespaceInsideFragment,
								removeTrailingWhitespaceInsideFragment);
						}
					}
	...
	function readAttributeValue(parser) {
		...
		value = readQuotedAttributeValue(parser, "'") || readQuotedAttributeValue(
			parser, "\"") || readUnquotedAttributeValue(parser);
	...
	function readUnquotedAttributeValue(parser) {
		...
		token = converters_readMustache(parser) || readUnquotedAttributeValueToken(
			parser);
		while (token !== null) {
			tokens.push(token);
			token = converters_readMustache(parser) ||
				readUnquotedAttributeValueToken(parser);
		}
	...
	function readQuotedAttributeValue(parser, quoteMark) {
		...
		token = converters_readMustache(parser) || readQuotedStringToken(parser,
			quoteMark);
		while (token !== null) {
			tokens.push(token);
			token = converters_readMustache(parser) || readQuotedStringToken(parser,
	...
	var JsonParser, specials, specialsPattern, parseJSON__numberPattern,
		placeholderPattern, placeholderAtStartPattern, onlyWhitespace;
	...
	function processDirective(tokens, parentParser) {
		var result, match, parser, args, token, colonIndex, directiveName,
			directiveArgs, parsed;
		...
		if (typeof tokens === "string") {
			if (match = methodCallPattern.exec(tokens)) {
				...
				if (!methodCallExcessPattern.test(tokens)) {
					parentParser.error("Invalid input after method call expression '" +
						tokens.slice(end + 1) + "'");
				}
	...
	disallowedContents = {
		...
		dt: ["dt", "dd"],
		dd: ["dt", "dd"],
		p: "address article aside blockquote div dl fieldset footer form h1 h2 h3 h4 h5 h6 header hgroup hr main menu nav ol p pre section table ul"
			.split(" "),
		rt: ["rt", "rp"],
		rp: ["rt", "rp"],
		optgroup: ["optgroup"],
	...
	function readElement(parser) {
		var start, element, directiveName, match, addProxyEvent, attribute,
			directive, selfClosing, children, partials, hasPartials, child, closed,
			pos, remaining, closingTag;
		...
		while (attribute = converters_readMustache(parser) ||
			element_readAttribute(parser)) {
			// regular attributes
			if (attribute.name) {
				// intro, outro, decorator
				if (directiveName = directives[attribute.name]) {
					element[directiveName] = element_processDirective(attribute.value,
						parser);
				}
				...
				// on-click etc
					...
					if (!parser.sanitizeEventAttributes || !onPattern.test(attribute.name)) {
						...
						element.a[attribute.name] = attribute.value || (attribute.value === "" ?
							"" : 0);
					}
				}
			}
			...
			do {
				...
				// implicit close by closing section tag. TODO clean this up
					...
					if (child = parser.read(PARTIAL_READERS)) {
						...
						utils_cleanup(child.f, parser.stripComments, preserveWhitespace, !
		...
		if (parser.sanitizeElements && parser.sanitizeElements.indexOf(
				lowerCaseName) !== -1) {
			return exclude;
		}
	...
	function readText(parser) {
		...
		return parser.inside ? remaining.substr(0, index) :
			decodeCharacterReferences(remaining.substr(0, index));
	}
	...
	function readPartialDefinitionComment(parser) {
		...
		var endPattern = new RegExp("^<!--\\s*" + utils_escapeRegExp(open) +
			"\\s*\\/\\s*" + name + "\\s*" + utils_escapeRegExp(close) + "\\s*-->");
		...
		do {
			if (parser.matchPattern(endPattern)) {
				...
				child = parser.read(READERS);
				if (!child) {
					parser.error("expected closing comment ('<!-- " + open + "/" + name +
						"" + close + " -->')");
				}
	...
	function readTemplate(parser) {
		...
		while (parser.pos < parser.str.length) {
			...
			if (partial = parser.read(PARTIAL_READERS)) {
				...
				utils_cleanup(partial.f, parser.stripComments, preserveWhitespace, !
					preserveWhitespace, !preserveWhitespace);
	...
	var STANDARD_READERS = [mustache_readPartial, mustache_readUnescaped,
		mustache_readSection, mustache_readYielder, mustache_readInterpolator,
		readMustacheComment
	];
	var TRIPLE_READERS = [mustache_readTriple];
	var STATIC_READERS = [mustache_readUnescaped, mustache_readSection,
	...
	var READERS = [converters_readMustache, converters_readHtmlComment,
		converters_readElement, converters_readText
	];
	var PARTIAL_READERS = [converters_readPartialDefinitionComment,
	...
	StandardParser = parse_Parser.extend({
		...
		postProcess: function(result) {
			...
			utils_cleanup(result[0].t, this.stripComments, this.preserveWhitespace, !
				this.preserveWhitespace, !this.preserveWhitespace);
	...
	var parseOptions = ["preserveWhitespace", "sanitize", "stripComments",
		"delimiters", "tripleDelimiters", "interpolate"
	];
	...
	function fromId(id, options) {
		...
		if (!isClient) {
			...
			throw new Error("Cannot retrieve template #" + id +
		...
		if (template.tagName.toUpperCase() !== "SCRIPT") {
			...
			throw new Error("Template element with id #" + id +
	...
	function getDynamicTemplate(ractive, fn) {
		var helper = template_template__createHelper(template_parser.getParseOptions(
			ractive));
	...
	function parseIfString(template, ractive) {
		...
		// Check we're using the correct version
			...
			throw new Error("Mismatched template version (expected " +
				TEMPLATE_VERSION + ", got " + template.v +
	...
	config_registries__registryNames = ["adaptors", "components", "computed",
		"decorators", "easing", "events", "interpolators", "partials",
		"transitions"
	];
	...
	Registry.prototype = {
		...
		extend: function(Parent, proto, options) {
			this.configure(this.useDefaults ? Parent.defaults : Parent, this.useDefaults ?
				proto : proto.constructor, options);
		},
   ...
   options );*/
	...
	function getMessage(deprecated, correct, isError) {
		return "options." + deprecated +
			" has been deprecated in favour of options." + correct + "." + (isError ?
	...
	function processItems(items, values, guid, counter) {
		...
		return items.map(function(item) {
			...
			if (item.keypath && (wrapped = item.root.viewmodel.wrapped[item.keypath
					.str])) {
				value = wrapped.value;
			} else {
				value = item.getValue();
	...
	SpecialResolver.prototype = {
		rebind: function() {
			...
			if (this.cached) {
				return this.callback(getKeypath("@" + prop.prefix + getProp(this.cached,
					prop)));
			}
			...
			if (prop.prop.indexOf("index") !== -1 || prop.prop.indexOf("key") !== -1) {
				while (fragment) {
					if (fragment.owner.currentSubtype === SECTION_EACH && (value = getProp(
							fragment, prop)) !== undefined) {
					...
					if (!fragment.parent && fragment.owner && fragment.owner.component &&
						fragment.owner.component.parentFragment && !fragment.owner.component.instance
						.isolated) {
	...
	var MemberResolver = function(template, resolver, parentFragment) {
		...
		// Simple reference?
			...
			this.refResolver = Resolvers_createReferenceResolver(this, template.n,
				function(keypath) {
		...
		// Otherwise we have an expression in its own right
			...
			new Resolvers_ExpressionResolver(resolver, parentFragment, template,
				function(keypath) {
					_this.resolve(keypath);
				});
		}
	...
	var ReferenceExpressionResolver = function(mustache, template, callback) {
		...
		if (keypath = shared_resolveRef(ractive, ref, parentFragment)) {
			...
			this.baseResolver = new Resolvers_ReferenceResolver(this, ref, function(
				keypath) {
				_this.base = keypath;
		...
		this.members = template.m.map(function(template) {
			return new ReferenceExpressionResolver_MemberResolver(template, _this,
				parentFragment);
		});
	...
	function ReferenceExpressionResolver_ReferenceExpressionResolver__getValue(
		member) {
		return member.value;
	}
	...
	function Mustache$init(mustache, options) {
		...
		if (ref = template.r) {
			mustache.resolver = Resolvers_createReferenceResolver(mustache, ref,
				resolve);
		}

		// if it's an expression, we have a bit more work to do
		if (options.template.x) {
			mustache.resolver = new Resolvers_ExpressionResolver(mustache,
				parentFragment, options.template.x, resolveAndRebindChildren);
		}

		if (options.template.rx) {
			mustache.resolver = new ReferenceExpressionResolver_ReferenceExpressionResolver(
				mustache, options.template.rx, resolveAndRebindChildren);
		}

		// Special case - inverted sections
		if (mustache.template.n === SECTION_UNLESS && !mustache.hasOwnProperty(
				"value")) {
	...
	function Section$shuffle(newIndices) {
		...
		var parentFragment, firstChange, i, newLength, reboundFragments,
		...
		if (section.subtype) {
			switch (section.subtype) {
				case SECTION_IF:
					...
					return reevaluateConditionalSection(section, value, false,
						fragmentOptions);

				case SECTION_UNLESS:
					section.hasContext = false;
					return reevaluateConditionalSection(section, value, true,
						fragmentOptions);
				...
				case SECTION_IF_WITH:
					return reevaluateConditionalContextSection(section, value,
						fragmentOptions);
	...
	function reevaluateListSection(section, value, fragmentOptions) {
		...
		if (length < section.length) {
			section.fragmentsToUnrender = section.fragments.splice(length, section.length -
				length);
	...
	function reevaluateConditionalSection(section, value, inverted,
		fragmentOptions) {
	...
	function removeSectionFragments(section) {
		if (section.length) {
			section.fragmentsToUnrender = section.fragments.splice(0, section.fragments
				.length).filter(isRendered);
			section.fragmentsToUnrender.forEach(methodCallers__unbind);
	...
	function Section$unrender(shouldDestroy) {
		this.fragments.forEach(shouldDestroy ? unrenderAndDestroy :
			prototype_unrender__unrender);
	...
	function element(tagName) {
		return elementCache[tagName] || (elementCache[tagName] = createElement(
			tagName));
	}
	...
	function Triple$update() {
		...
		parentNode.insertBefore(this.docFrag, this.parentFragment.findNextNode(
			this));
	...
	function Attribute$bubble() {
		var value = this.useProperty || !this.rendered ? this.fragment.getValue() :
	...
	var determineNameAndNamespace = function(attribute, name) {
		...
		attribute.name = attribute.element.namespace !== namespaces.html ?
			enforceCase(name) : name;
	};
	...
	function Attribute$toString() {
		...
		if (name === "value" && (this.element.name === "select" || this.element.name ===
				"textarea")) {
			return;
		}

		// Special case - content editable
		if (name === "value" && this.element.getAttribute("contenteditable") !==
			undefined) {
			return;
		}
		...
		return value ? name + "=\"" + Attribute_prototype_toString__escape(value) +
			"\"" : name;
	}
	...
	function Attribute$updateRadioValue() {
		...
		node.checked = this.element.getAttribute("value") === this.element.getAttribute(
			"name");
	...
	function Attribute$update() {
		...
		if (name === "id") {
			...
			if (element.name === "select" && name === "value") {
				updateMethod = element.getAttribute("multiple") ?
			...
			// special case - <input>
				...
				else if (type === "radio" && element.binding && element.binding.name ===
					"name") {
		...
		else if (name === "class" && (!node.namespaceURI || node.namespaceURI ===
				namespaces.html)) {
		...
		if (keypath = interpolator.keypath) {
			if (keypath.str.slice(-1) === "}") {
				warnOnceIfDebug(
					...
					interpolator.resolver.uniqueString, element.name, {
						ractive: this.root
					});
				return false;
			}
			...
			if (keypath.isSpecial) {
				warnOnceIfDebug("Two-way binding does not work with %s", interpolator.resolver
					.ref, {
						ractive: this.root
					});
				return false;
			...
			var ref = interpolator.template.r ? "'" + interpolator.template.r +
	...
	Binding.prototype = {
		...
		rebound: function() {
			...
			bindings = this.root._twowayBindings[newKeypath.str] || (this.root._twowayBindings[
				newKeypath.str] = []);
			bindings.push(this);
		},
	...
	var RadioBinding = Binding_Binding.extend({
		...
		init: function() {
			this.siblings = shared_getSiblings(this.root._guid, "radio", this.element
				.getAttribute("name"));
	...
	var RadioNameBinding = Binding_Binding.extend({
		...
		init: function() {
			this.siblings = shared_getSiblings(this.root._guid, "radioname", this.keypath
				.str);
			this.siblings.push(this);
		...
		render: function() {
			...
			node.checked = this.root.viewmodel.get(this.keypath) == this.element.getAttribute(
				"value");
	...
	var CheckboxNameBinding = Binding_Binding.extend({
		...
		init: function() {
			...
			this.siblings = shared_getSiblings(this.root._guid, "checkboxes", this.keypath
				.str);
			this.siblings.push(this);
		...
		getValue: function() {
			return this.siblings.filter(isChecked).map(
				Binding_CheckboxNameBinding__getValue);
		}
	});
	...
	var MultipleSelectBinding = Binding_SelectBinding.extend({
		...
		handleChange: function() {
			...
			if (previousValue === undefined || !arrayContentsMatch(value,
					previousValue)) {
	...
	function createTwowayBinding(element) {
		...
		if (
			...
			(element.getAttribute("contenteditable") || !!attributes.contenteditable &&
				isBindable(attributes.contenteditable)) && isBindable(attributes.value)) {
		...
		// <input>
			...
			if (type === "radio" || type === "checkbox") {
				...
				if (bindName) {
					Binding = type === "radio" ? Binding_RadioNameBinding :
						Binding_CheckboxNameBinding;
				} else if (bindChecked) {
					Binding = type === "radio" ? Binding_RadioBinding :
						Binding_CheckboxBinding;
				}
			} else if (type === "file" && isBindable(attributes.value)) {
				Binding = Binding_FileListBinding;
			} else if (isBindable(attributes.value)) {
				Binding = type === "number" || type === "range" ? NumericBinding :
		...
		// <select>
			...
			Binding = element.getAttribute("multiple") ?
				Binding_MultipleSelectBinding : Binding_SelectBinding;
		}
		...
		if (template.m) {
			...
			refs.forEach(function(ref, i) {
				...
				if (match = eventPattern.exec(ref)) {
					...
					_this.refResolvers.push(Resolvers_createReferenceResolver(_this, ref,
						function(keypath) {
	...
	function sync(selectElement) {
		...
		if (selectValue !== undefined) {
			options.forEach(function(o) {
				...
				shouldSelect = isMultiple ? valueContains(selectValue, optionValue) :
	...
	function special_option__init(option, template) {
		...
		if (template.a.value === undefined && !template.a.hasOwnProperty(
				"disabled")) {
		...
		if ("selected" in template.a && option.select.getAttribute("value") !==
			undefined) {
	...
	function Element$init(options) {
		...
		if (twoway && (binding = init_createTwowayBinding(this, template.a))) {
			...
			bindings = this.root._twowayBindings[binding.keypath.str] || (this.root._twowayBindings[
				binding.keypath.str] = []);
	...
	if (!isClient) {
		...
		(function() {
			...
			} else if (animateStyle_createTransitions__testStyle.webkitTransition !==
				undefined) {
		...
		createTransitions = function(t, to, options, changedProperties, resolve) {
			...
			setTimeout(function() {
				...
				var hashPrefix, jsTransitionsComplete, cssTransitionsComplete,
					checkComplete, transitionEndHandler;
				...
				t.node.style[TRANSITION_PROPERTY] = changedProperties.map(
					helpers_prefix).map(hyphenate).join(",");
				t.node.style[TRANSITION_TIMING_FUNCTION] = hyphenate(options.easing ||
					"linear");
				...
				transitionEndHandler = function(event) {
					...
					t.node.removeEventListener(TRANSITIONEND, transitionEndHandler,
						false);
					...
					while (i--) {
						...
						if (!CSS_TRANSITIONS_ENABLED || cannotUseCssTransitions[hash]) {
							...
							propertiesToTransitionInJs.push({
								...
								interpolator: shared_interpolate(parseFloat(originalValue),
									parseFloat(to[prop])),
								suffix: suffix
							});
						}
	...
	var hidden, vendor, animateStyle_visibility__prefix,
		animateStyle_visibility__i, visibility;
	...
	if (!isClient) {
		...
		animateStyle = function(style, value, options) {
			...
			var promise = new utils_Promise(function(resolve) {
				var propertyNames, changedProperties, computedStyle, current, from, i,
					prop;
				...
				animateStyle_createTransitions(_this, to, options, changedProperties,
					resolve);
			});
	...
	function Element$render() {
		...
		if (this.parentFragment.cssIds) {
			this.node.setAttribute("data-ractive-css", this.parentFragment.cssIds.map(
				function(x) {
					return "{" + x + "}";
				}).join(" "));
		}
	...
	var Element_prototype_toString = function() {
		...
		str += this.attributes.map(stringifyAttribute).join("") + this.conditionalAttributes
			.map(stringifyAttribute).join("");
	...
	function inputIsCheckedRadio(element) {
		...
		if (!typeAttribute || typeAttribute.value !== "radio" || !valueAttribute ||
			!nameAttribute.interpolator) {
			return;
		}
	...
	function getPartialTemplate(ractive, name, parentFragment) {
		...
		if (partial) {
			...
			var parsed = template_parser.parse(partial, template_parser.getParseOptions(
				ractive));
	...
	function getPartialFromRegistry(ractive, name, parentFragment) {
		...
		if (!template_parser.isParsed(partial)) {
			...
			if (parsed.p) {
				warnIfDebug("Partials ({{>%s}}) cannot contain nested inline partials",
					name, {
						ractive: ractive
					});
			}
	...
	function findOwner(ractive, key) {
		return ractive.partials.hasOwnProperty(key) ? ractive : findConstructor(
			ractive.constructor, key);
	}
	...
	function findConstructor(constructor, key) {
		...
		return constructor.partials.hasOwnProperty(key) ? constructor :
	...
	var Partial = function(options) {
		...
		if (!this.keypath) {
			if (template = Partial_getPartialTemplate(this.root, this.name,
					parentFragment)) {
	...
	Partial.prototype = {
		...
		setValue: function(value) {
			...
			if (!template && this.name && (template = Partial_getPartialTemplate(
					this.root, this.name, this.parentFragment))) {
				shared_unbind.call(this);
				this.isNamed = true;
			}
	...
	array_index__errorMessage =
		"Something went wrong in a rather interesting way";
	...
	try {
		...
		magicAdaptor = {
			filter: function(object, keypath, ractive) {
				...
				if ((parentWrapper = ractive.viewmodel.wrapped[keypath.parent.str]) &&
					!parentWrapper.magic) {
					return false;
				}
				...
				return parentValue && (typeof parentValue === "object" || typeof parentValue ===
					"function");
			},
		...
		MagicWrapper = function(ractive, value, keypath) {
			...
			this.obj = objKeypath.isRoot ? ractive.viewmodel.data : ractive.viewmodel
				.get(objKeypath);

			template = this.originalDescriptor = Object.getOwnPropertyDescriptor(this
				.obj, this.prop);
	...
	function createAccessors(originalWrapper, value, template) {
		...
		if (template && !template.configurable) {
			...
			throw new Error("Cannot use magic mode with property \"" + property +
	...
	if (adaptors_magic) {
		magicArrayAdaptor = {
			filter: function(object, keypath, ractive) {
				return adaptors_magic.filter(object, keypath, ractive) && array_index.filter(
					object);
			},
	...
	function Viewmodel$adapt(keypath, value) {
		...
		for (i = 0; i < len; i += 1) {
			...
			if (adaptor.filter(value, keypath, this.ractive)) {
				wrapped = this.wrapped[keypath] = adaptor.wrap(this.ractive, value,
					keypath, getPrefixer(keypath));
				wrapped.value = value;
				return;
			}
		}
		...
		if (!prefixers[rootKeypath]) {
			...
			prefixers[rootKeypath] = function(relativeKeypath, value) {
				...
				if (typeof relativeKeypath === "object") {
					...
					return rootDot ? prefixKeypath(relativeKeypath, rootKeypath) :
						relativeKeypath;
				}
			};
		}
	...
	function Viewmodel$applyChanges() {
		...
		upstreamChanges.forEach(function(keypath) {
			...
			if (changes.indexOf(keypath) === -1 && (computations = self.deps.computed[
					keypath.str])) {
				computations.forEach(invalidateComputation);
			}
		});
	...
	Computation.prototype = {
		...
		updateDependencies: function(newDeps) {
			...
			while (i--) {
				...
				if (oldDeps.indexOf(keypath) === -1 && (!this.hardDeps || this.hardDeps
						.indexOf(keypath) === -1)) {
					dependenciesChanged = true;
	...
	function isUnresolved(viewmodel, keypath) {
		...
		return !(key in viewmodel.data) && !(key in viewmodel.computations) && !(
			key in viewmodel.mappings);
	}
	...
	function Viewmodel$merge(keypath, currentArray, array, options) {
		...
		this.smartUpdate(keypath, array, newIndices, currentArray.length !== array
			.length);
	}
	...
	function createFunctionFromString(ractive, str) {
		...
		functionBody = "return (" + str.replace(
			helpers_getComputationSignatures__pattern,
	...
	var initialise__registryNames = ["adaptors", "components", "decorators",
		"easing", "events", "interpolators", "partials", "transitions"
	];
	...
	function initialiseRactiveInstance(ractive) {
		...
		initialise__registryNames.forEach(function(name) {
			ractive[name] = utils_object__extend(create(ractive.constructor[name] ||
				null), userOptions[name]);
		});
		...
		viewmodel = new viewmodel_Viewmodel({
			...
			computed: helpers_getComputationSignatures(ractive,
				utils_object__extend(create(ractive.constructor.prototype.computed),
	...
	function getAdaptors(ractive, protoAdapt, userOptions) {
		...
		modifyArrays = "modifyArrays" in userOptions ? userOptions.modifyArrays :
			ractive.modifyArrays;
	...
	var createInstance = function(component, Component, attributes,
		yieldTemplate, partials) {
		var instance,
			parentFragment,
			ractive,
			fragment,
			...
			Object.keys(attributes).forEach(function(key) {
				...
				if (typeof attribute === "string") {
					...
					if (isSingleInterpolator(attribute)) {
						...
						resolver = new initialise_ComplexParameter(component, attribute,
							function(value) {
								if (ready) {
	...
	function createResolver(component, template, callback) {
		...
		if (template.r) {
			resolver = Resolvers_createReferenceResolver(component, template.r,
				callback);
		} else if (template.x) {
			resolver = new Resolvers_ExpressionResolver(component, component.parentFragment,
				template.x, callback);
		} else if (template.rx) {
			resolver = new ReferenceExpressionResolver_ReferenceExpressionResolver(
				component, template.rx, callback);
		}
	...
	function propagateEvents(component, eventsDescriptor) {
		...
		for (eventName in eventsDescriptor) {
			if (eventsDescriptor.hasOwnProperty(eventName)) {
				propagateEvent(component.instance, component.root, eventName,
					eventsDescriptor[eventName]);
			}
		}
	}

	function propagateEvent(childInstance, parentInstance, eventName,
		proxyEventName) {
	...
	function Component$init(options, Component) {
		...
		createInstance(this, Component, options.template.a, options.template.f,
			options.template.p);
		initialise_propagateEvents(this, options.template.v);

		// intro, outro and decorator directives have no effect
		if (options.template.t0 || options.template.t1 || options.template.t2 ||
			options.template.o) {
			warnIfDebug(
	...
	var Yielder = function(options) {
		...
		global_runloop.scheduleTask(function() {
			if (component.yielders[name].length > 1) {
				throw new Error("A component template can only have one {{yield" + (
					name ? " " + name : "") + "}} declaration at a time");
			}
		});
	};
	...
	function Fragment$init(options) {
		...
		this.cssIds = "cssIds" in options ? options.cssIds : this.parent ? this.parent
			.cssIds : null;
	...
	function createItem(options) {
		...
		switch (options.template.t) {
			...
			case ELEMENT:
				...
				if (constructor = Component_getComponent(options.parentFragment.root,
						options.template.e)) {
	...
	function Fragment$toString(escape) {
		...
		return this.items.map(escape ? toEscapedString :
			Fragment_prototype_toString__toString).join("");
	}
	...
	var shouldRerender = ["template", "partials", "components", "decorators",
			"events"
		],
	...
	function Ractive$toggle(keypath) {
		...
		if (/\*/.test(keypath)) {
			...
			getMatchingKeypaths(this, getKeypath(normalise(keypath))).forEach(
				function(keypath) {
	...
	function addOtherOptions(Child, options) {
		Object.keys(Child.prototype).forEach(function(key) {
			...
			else if (typeof options[key] === "function" && typeof value ===
				"function" && options[key]._method) {
	...
	function extendOne(Parent) {
		...
		if (options.computed) {
			proto.computed = utils_object__extend(create(Parent.prototype.computed),
				options.computed);
		}
	...
	if (typeof Date.now !== FUNCTION || typeof String.prototype.trim !==
		FUNCTION || typeof Object.keys !== FUNCTION || typeof Array.prototype.indexOf !==
		FUNCTION || typeof Array.prototype.forEach !== FUNCTION || typeof Array.prototype
		.map !== FUNCTION || typeof Array.prototype.filter !== FUNCTION || typeof window !==
		"undefined" && typeof window.addEventListener !== FUNCTION) {
		throw new Error(
...
(function(global, factory) {
 ...
 if (typeof module !== 'undefined' && module.exports && typeof require ===
  'function') {
 ...
 // browser global
  ...
  throw new Error(
   'Could not find Ractive! It must be loaded before the ractive-events-keys plugin'
  );
 }
 ...
 console.log('loaded keys', Ractive);
...
(function(global, factory) {
 ...
 // browser global
  ...
  throw new Error(
   'Could not find Ractive! It must be loaded before the ractive-events-typing plugin'
  );
 }
 ...
 var typing = function(node, fire) {
  ...
  var eligibleForTyping = ({
   ...
   "input": 0,
   "textareatextarea": 0
  })[[(node.tagName || "").toLowerCase(), (node.type || "").toLowerCase()].join(
   '')] === 0 || typeof(node.contentEditable) != 'undefined';
...
(function(global, factory) {
	...
	// browser global
		...
		throw new Error(
			'Could not find Ractive! It must be loaded before the ractive-transitions-fade plugin'
		);
	}
...
(function(global, factory) {
 ...
 // browser global
  ...
  throw new Error(
   'Could not find Ractive! It must be loaded before the ractive-transitions-slide plugin'
  );
 }
 ...
 console.log('slide loaded', Ractive);
}));
...
(function(global, factory) {
	...
	// browser global
		...
		throw new Error(
			'Could not find Ractive! It must be loaded before the ractive-transitions-fly plugin'
		);
	}
...
(function(global, factory) {
 ...
 // browser global
  ...
  throw new Error(
   'Could not find Ractive! It must be loaded before the Ractive-transitions-scale plugin'
  );
 }
...
(function(global, factory) {
	...
	// browser global
		...
		throw new Error(
			'Could not find Ractive! It must be loaded before the Ractive-adaptors-Promise plugin'
		);
	}
