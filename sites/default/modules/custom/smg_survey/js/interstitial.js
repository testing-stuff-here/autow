...
// Find out if it is home page or interior page
path = window.location.pathname;
if(path == '/' || path == '/home') {
  page = 'home';
}
else if(path == '/demo/survey') {
  page = 'demo';
}
else {
  page = 'interior';
...
//displayfiles: ['/blackmesh/pw/sites/default/modules/oas/interstitialIframe.inc'],
displayfiles: ['/smg-survey/'],

//2) display freqency: ["frequency_type", "frequency_value"]
displayfrequency: ["cookie", "180"],

//3) HTML for the header bar portion of the interstitial box
defineheader: '<div class="headerbar"><a href="#" onclick="ga(\'send\', \'event\', \'Print Subscriber Poll\', \'Close Box No Answer\'); javascript:interstitialBox.closeit(); return false"><img src="/sites/default/modules/custom/smg_survey/css/images/x.png" style="border: 0" title="Close Box"/></a></div>',

//4) cookie setting: ["cookie_name", "cookie_path"]
cookiesetting: ["smgsurvey", "path=/"],
...
if (getCookie(stitialvars.cookiename) == null ||
parseInt(getCookie(stitialvars.cookiename)) != parseInt(stitialvars.cookieduration) ||
window.location.pathname == interstitialBox.demoUrl){
//if persistent cookie is empty or admin has changed number of days
//to persist from that of the stored value (meaning, reset it)
