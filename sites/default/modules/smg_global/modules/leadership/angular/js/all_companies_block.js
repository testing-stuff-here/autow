...
all_companies_block.directive('companyList', function () {
  return {
    restrict: 'E',
    replace: true,
    template: '<li ng-repeat="company in taxonomy.companies | orderObjectBy:companyListOrder" leadership-link>' + '<a class="all-companies-block-text company-name leadership-link" href="{{company.alias}}">{{company.title}}</a>' + '<span class="youtube-amplified" ng-show="hasPmgYoutubeFeedVideos(company)">' + '<span class="youtube-amplified-video-icon"></span>' + '</span>' + '<compile-data-card tid="{{company.tid}}" nid="{{company.datacard_nid}}" site="{{site}}" hover-state="hoverState"></compile-data-card>' + '</li>'
  };
});
...
all_companies_block.directive('taxonomyList', ['$timeout', function ($timeout) {
  return {
    ...
    template: '<div class="taxonomy-tid-{{taxonomy.tid}} taxonomy-tid">' + '<span class="brackets" ng-click="categoryOpen(taxonomy)">' + '[' + '<span class="plus-minus">' + '<span ng-show="!taxonomy.show">+</span><span ng-show="taxonomy.show">–</span>' + '</span>' + ']' + '</span>' + '&nbsp;' + '<span class="taxonomy-name all-companies-block-text" ng-click="categoryOpen(taxonomy)" ng-bind="taxonomy.name" ng-class="{loading: taxonomy.loading}"></span>' + '<ul class="company-list leadership-menu hide" ng-class="{slideDown: taxonomy.show, slideUp: !taxonomy.show}">' + '<company-list></company-list>' + '</ul>' + '</div>'
  };
}]);
